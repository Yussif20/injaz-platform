...
export async function POST(
  ...
  try {
    ...
    const response = await serverApi.post<ApiResponse<boolean>>(
      `${API_ENDPOINTS.MY_PROFILES}/${id}/unpublish`,
      null,
      {
        headers: {
    ...
    if (isApiSuccess(response.data.status)) {
      return NextResponse.json({
        status: true,
        message: response.data.message || "تم إلغاء نشر الملف بنجاح",
      });
    }
    ...
    return NextResponse.json(
      {
        status: false,
        message: response.data.message || "فشل في إلغاء نشر الملف",
    ...
    if (error && typeof error === "object" && "response" in error) {
      ...
      return NextResponse.json(
        {
          status: false,
          message: errorData?.message || "فشل في إلغاء نشر الملف",

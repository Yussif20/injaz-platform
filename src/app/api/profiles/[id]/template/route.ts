/**
 * Profile Template API routes
 * PUT /api/profiles/[id]/template - Update profile template
 */
...
export async function PUT(
  ...
  try {
    ...
    const response = await serverApi.put<ApiResponse<Profile>>(
      `${API_ENDPOINTS.MY_PROFILES}/${id}/template`,
      body,
      {
        headers: {
    ...
    if (isApiSuccess(response.data.status)) {
      return NextResponse.json({
        status: true,
        message: response.data.message || "تم تحديث القالب بنجاح",
    ...
    return NextResponse.json(
      {
        status: false,
        message: response.data.message || "فشل في تحديث القالب",
    ...
    if (error && typeof error === "object" && "response" in error) {
      ...
      return NextResponse.json(
        {
          status: false,
          message: errorData?.message || "فشل في تحديث القالب",

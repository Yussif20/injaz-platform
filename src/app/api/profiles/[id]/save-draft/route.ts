/**
 * Profile Save Draft API routes
 * POST /api/profiles/[id]/save-draft - Save profile as draft
 */
...
export async function POST(
  ...
  try {
    ...
    const response = await serverApi.post<ApiResponse<boolean>>(
      `${API_ENDPOINTS.MY_PROFILES}/${id}/save-draft`,
      null,
      {
        headers: {
    ...
    if (isApiSuccess(response.data.status)) {
      return NextResponse.json({
        status: true,
        message: response.data.message || "تم حفظ المسودة بنجاح",
      });
    }
    ...
    return NextResponse.json(
      {
        status: false,
        message: response.data.message || "فشل في حفظ المسودة",
    ...
    if (error && typeof error === "object" && "response" in error) {
      ...
      return NextResponse.json(
        {
          status: false,
          message: errorData?.message || "فشل في حفظ المسودة",

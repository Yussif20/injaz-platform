/**
 * Profile Types API routes
 * GET /api/profile-types - Get available profile types (gender-filtered)
 */
...
/**
 * GET /api/profile-types - Get available profile types for current user
 */
export async function GET() {
  try {
    ...
    const response = await serverApi.get<ApiResponse<ProfileType[]>>(
      API_ENDPOINTS.PROFILE_TYPES_AVAILABLE,
      {
        headers: {
    ...
    if (isApiSuccess(response.data.status)) {
      return NextResponse.json({
        status: true,
        message: response.data.message || "تم جلب أنواع الملفات بنجاح",
    ...
    return NextResponse.json(
      {
        status: false,
        message: response.data.message || "فشل في جلب أنواع الملفات",
    ...
    if (error && typeof error === "object" && "response" in error) {
      ...
      return NextResponse.json(
        {
          status: false,
          message: errorData?.message || "فشل في جلب أنواع الملفات",

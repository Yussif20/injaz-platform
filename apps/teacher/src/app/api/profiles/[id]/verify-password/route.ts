/**
 * Profile Verify Password API routes
 * POST /api/profiles/[id]/verify-password - Verify profile password
 */
...
export async function POST(
  ...
  try {
    ...
    if (isApiSuccess(response.data.status)) {
      return NextResponse.json({
        status: true,
        message: response.data.message || "كلمة المرور صحيحة",
        data: { isValid: true },
      });
    }
    ...
    return NextResponse.json(
      {
        status: false,
        message: response.data.message || "كلمة المرور غير صحيحة",
        data: { isValid: false },
    ...
    if (error && typeof error === "object" && "response" in error) {
      ...
      return NextResponse.json(
        {
          status: false,
          message: errorData?.message || "فشل في التحقق من كلمة المرور",

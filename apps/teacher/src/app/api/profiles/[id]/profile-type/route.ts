/**
 * Profile Type API routes
 * PUT /api/profiles/[id]/profile-type - Update profile type
 */
...
export async function PUT(
  ...
  try {
    ...
    const response = await serverApi.put<ApiResponse<Profile>>(
      `${API_ENDPOINTS.MY_PROFILES}/${id}/profile-type`,
      body,
      {
        headers: {
    ...
    if (isApiSuccess(response.data.status)) {
      return NextResponse.json({
        status: true,
        message: response.data.message || "تم تحديث نوع الملف بنجاح",
    ...
    return NextResponse.json(
      {
        status: false,
        message: response.data.message || "فشل في تحديث نوع الملف",
    ...
    if (error && typeof error === "object" && "response" in error) {
      ...
      return NextResponse.json(
        {
          status: false,
          message: errorData?.message || "فشل في تحديث نوع الملف",

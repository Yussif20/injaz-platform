...
export async function GET() {
  try {
    ...
    const response = await serverApi.get<ApiResponse<Qualification[]>>(
      API_ENDPOINTS.MY_QUALIFICATIONS,
      {
        headers: {
    ...
    if (isApiSuccess(response.data.status)) {
      return NextResponse.json({
        status: true,
        message: response.data.message || "تم جلب المؤهلات بنجاح",
    ...
    return NextResponse.json(
      {
        status: false,
        message: response.data.message || "فشل في جلب المؤهلات",
    ...
    if (error && typeof error === "object" && "response" in error) {
      ...
      return NextResponse.json(
        {
          status: false,
          message: errorData?.message || "فشل في جلب المؤهلات",
...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await serverApi.post<ApiResponse<Qualification>>(
      API_ENDPOINTS.MY_QUALIFICATIONS,
      body,
      {
        headers: {
    ...
    if (isApiSuccess(response.data.status)) {
      return NextResponse.json({
        status: true,
        message: response.data.message || "تمت إضافة المؤهل بنجاح",
        data: response.data.data,
      });
    }
    ...
    return NextResponse.json(
      {
        status: false,
        message: response.data.message || "فشل في إضافة المؤهل",
        errors: response.data.errors,
      },
      { status: 400 }
    );
    ...
    if (error && typeof error === "object" && "response" in error) {
      ...
      return NextResponse.json(
        {
          status: false,
          message: errorData?.message || "فشل في إضافة المؤهل",
          errors: errorData?.errors || null,
        },
        { status: statusCode }
      );
